...
import views.special_parameters
import views.what_is_fact
...
def doc_layout_router(pathname):
    ...
    # 处理针对非常规单页组件的路由请求
    if pathname in ['/what-is-fact', '/']:
        doc_layout = views.what_is_fact.render()

    elif pathname == '/style':
        doc_layout = views.special_parameters.style_configuration_items.render()
    ...
    valid_views = [
        ...
        if not name.startswith('__') and name != 'special_parameters'
    ]
